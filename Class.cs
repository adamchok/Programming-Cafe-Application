...
namespace APU_Programming_Cafe
{
    public class Class
    {
        ...
        private string _UpdatedClassCode;
        ...
        public string UpdatedClassCode
        {
            get { return _UpdatedClassCode; }
            set { _UpdatedClassCode = value; }
        }
        ...
        public Class(string classCode, string trainerID, string moduleCode, string intake, string timeSchedule, string day, string venue)
        {
            ClassCode = classCode;
            TrainerID = trainerID;
        ...
        public Class(string currentClassCode, string day, string timeSchedule, string venue, string updatedClassCode, string trainerID)
        {
        ...
        public Class(string classCode, string trainerID, string classCompletion)
        {
            ClassCode = classCode;
            TrainerID = trainerID;
        ...
        public List<string> GetOccupiedVenues(List<string> allVenues, int startTime, int endTime, string connectionString, string day)
        {
            List<string> occupiedVenues = new List<string>();

            foreach (string venue in allVenues)
            {
                // Perform your logic to fetch start and end time for the venue
                try
                {
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            if ((startTime <= start && endTime >= end) || (startTime >= start && endTime <= end))
                            {
        ...
        public bool updateClass(string connectionString)
        {
            try
            {
                ...
                string updateEnrolment = "UPDATE Enrolment SET ClassCode = @updatedClassCode WHERE TrainerID = @trainerID AND ClassCode = @currentClassCode;";
                SqlCommand updateEnrolmentCommand = new SqlCommand(updateEnrolment, connection);
                updateEnrolmentCommand.Parameters.AddWithValue("@updatedClassCode", _UpdatedClassCode);
                updateEnrolmentCommand.Parameters.AddWithValue("@trainerID", _TrainerID);
                updateEnrolmentCommand.Parameters.AddWithValue("@currentClassCode", _ClassCode);
                updateEnrolmentCommand.ExecuteNonQuery();

                string updateClassTable = "UPDATE Class SET ClassCode = @updatedClassCode, TimeSchedule = @updatedTimeSchedule, Day = @updatedDay, Venue = @updatedVenue WHERE TrainerID = @trainerID AND ClassCode = @currentClassCode;";
                SqlCommand updateClassTableCommand = new SqlCommand(updateClassTable, connection);
                updateClassTableCommand.Parameters.AddWithValue("@updatedClassCode", _UpdatedClassCode);
